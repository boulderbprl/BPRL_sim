...
%assert(state.delta_t<=max_timestep)
...
function state = init(state)
    ...
    state.accel_ef     = [0;0;0];   % body frame (m/s^2)
...
state.accel_ef = accel_ef + [0; 0; -state.gravity_mss];
...
[theta_des,phi_des,thrust_sp,controllers] = update_pos_vel_controller(state,target,controllers,dt);
...
if state.on_ground
    % Reset acc loop and rate loops while on ground.
    % vel_D is allowed to accumulate so thrust builds during takeoff.
    controllers.acc_D.reset();
...
MAX_VEL_Z     = 3.0;           % m/s  (magnitude, up is negative NED)
    ...
    % Vertical acceleration PI → thrust correction 
    az_ef       = state.accel_ef(3);
    thrust_corr = controllers.acc_D.update(az_cmd/g, az_ef/g, dt);
    thrust_sp   = clamp(HOVER_THRUST - thrust_corr, 0.0, 1.0);
    ...
    phi_des   = clamp(phi_des,   -MAX_LEAN, MAX_LEAN);
     ...
     % fprintf('  vz_cmd=%6.3f az_cmd=%6.3f az_ef=%6.3f thr=%5.3f pos_err=[%5.2f %5.2f %5.2f]\n', ...
     %        vz_cmd, az_cmd, az_ef, thrust_sp, ...
     %        target.position(1)-px, target.position(2)-py, target.position(3)-pz);
...
% Yaw error wrap (the class computes target-measurement; wrap the result)
% Recompute yaw cmd with wrapped error since class doesn't wrap angles
yaw_err = wrapToPi(yaw_des - psi);
r_cmd = controllers.att_yaw.Kp * yaw_err;
