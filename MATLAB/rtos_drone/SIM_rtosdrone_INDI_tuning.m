...
history = run_sim_finite(state, target_function, init_function, physics_function, control_update);
...
function target = nav_figure8(t)
 ...
 % Hover at (0, 0, -20 m) for first 15 s, then fly a figure-8
    fig8_alt = -20;          % NED: negative = up
    fig8_A   = 5;
    fig8_T   = 30;
    fig8_w   = 2*pi / fig8_T;
...
[torque_roll,torque_pitch,torque_yaw,pdot_cmd,qdot_cmd, controllers] = update_rate_controller(state,p_cmd,q_cmd,r_cmd,controllers,dt);

target.pdot = pdot_cmd; 
target.qdot = qdot_cmd;
target.rdot = 0;   % yaw has no accel setpoint in your current scheme

state.controller.tau_roll = torque_roll; 
state.controller.tau_pitch = torque_pitch; 
state.controller.tau_yaw = torque_yaw;
...
function c = build_controllers(copter)
    ...
    % Pull live gains from the tuner GUI if present, else use defaults
    if evalin('base','exist(''TUNER_GAINS'',''var'')')
        g = evalin('base','TUNER_GAINS');
    else
        g = struct();   % empty → all fields below fall back to literals
    end

    function v = G(field, default)
        if isfield(g, field); v = g.(field); else; v = default; end
    end

    % ── Position ─────────────────────────────────────────────────────────
    c.pos_N = controller_PID(G('pos_N_Kp',1.0), 0, 0, 0, 0, 0);
    c.pos_E = controller_PID(G('pos_E_Kp',1.0), 0, 0, 0, 0, 0);
    c.pos_D = controller_PID(G('pos_D_Kp',1.0), 0, 0, 0, 0, 0);

    % ── Velocity ─────────────────────────────────────────────────────────
    c.vel_N = controller_PID(G('vel_N_Kp',2.0), G('vel_N_Ki',1.0), G('vel_N_Kd',0.5), 0);
    c.vel_E = controller_PID(G('vel_E_Kp',2.0), G('vel_E_Ki',1.0), G('vel_E_Kd',0.5), 0);
    c.vel_D = controller_PID(G('vel_D_Kp',5.0), G('vel_D_Ki',2.0), G('vel_D_Kd',0.0), 0);

    % ── Altitude accel ───────────────────────────────────────────────────
    c.acc_D = controller_PID(0.75, 1.5, 0.0, 0);

    % ── Attitude ─────────────────────────────────────────────────────────
    c.att_roll  = controller_PID(G('att_roll_Kp', 4.5), 0, 0, 0);
    c.att_pitch = controller_PID(G('att_pitch_Kp',4.5), 0, 0, 0);
    c.att_yaw   = controller_PID(G('att_yaw_Kp',  4.5), 0, 0, 0);

    % ── Rates ────────────────────────────────────────────────────────────
    c.rate_roll  = controller_PID(G('rate_roll_Kp', 8.0), G('rate_roll_Ki', 0.0), ...
                                  G('rate_roll_Kd', 1.5),   0, 0.444, 20);
    c.rate_pitch = controller_PID(G('rate_pitch_Kp',8.0), G('rate_pitch_Ki',0.0), ...
                                  G('rate_pitch_Kd',1.5),   0, 0.444, 20);
    c.rate_yaw   = controller_PID(G('rate_yaw_Kp', 0.18),  G('rate_yaw_Ki', 0.018), ...
                                  G('rate_yaw_Kd', 0.0),    0, 0.222);

    % ── Gyro filter states ────────────────────────────────────────────────
    c.p_filt = 0.0;  c.q_filt = 0.0;  c.r_filt = 0.0;

    % ── INDI ─────────────────────────────────────────────────────────────
    gi = inv(copter.inertia);
    c.accel_roll_INDI  = controller_INDI(G('indi_roll_Kp', 0.35), @(x) gi(1,1));
    c.accel_pitch_INDI = controller_INDI(G('indi_pitch_Kp',0.35), @(x) gi(2,2));
end
...
function [torque_roll,torque_pitch,torque_yaw,pdot_cmd,qdot_cmd, controllers] = update_rate_controller(state,p_cmd,q_cmd,r_cmd,controllers,dt)
...
% normalize U2 and U3 
torque_roll  = clamp(U2/(2*state.copter.max_motor_arm_moment_roll) ,  -1, 1);
torque_pitch = clamp(U3/(2*state.copter.max_motor_arm_moment_pitch) ,   -1, 1);
...
function history = run_sim_finite(state, target_function, init_function, physics_function, control_update)
    ...
    tgt_pos_log  = zeros(3, n_steps);
    tgt_vel_log  = zeros(3, n_steps);
    tgt_att_log  = zeros(3, n_steps);   % phi, theta, yaw
    tgt_rate_log = zeros(3, n_steps);   % p, q, r
    tgt_accel_log= zeros(3, n_steps);   % pdot, qdot, rdot
    ...
    for k = 1:n_steps
        ...
        % ── Targets ───────────────────────────────────────────────────
        tgt_pos_log(:,k)   = target.position;
        tgt_vel_log(:,k)   = [target.vx; target.vy; target.vz];
        tgt_att_log(:,k)   = rad2deg([target.phi; target.theta; target.yaw]);
        tgt_rate_log(:,k)  = rad2deg([target.p; target.q; target.r]);
        tgt_accel_log(:,k) = rad2deg([target.pdot; target.qdot; target.rdot]);
