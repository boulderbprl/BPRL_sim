...
a = sysd.Denominator(2:end);   % denominator (feedback) coefficients
b = sysd.Numerator(2:end);   % numerator   (feedforward) coefficients
...
subplot(2,1,1);
plot(tw(interval), cmd(interval), 'Color', [0.5 0.5 0.5], 'LineWidth', 1.2);
...
subplot(2,1,2);
plot(tw(interval), thrw(interval),  'b',  'LineWidth', 1.5, 'DisplayName', 'Measured'); hold on;
plot(tw(interval), thrust_pred(interval), 'r--', 'LineWidth', 1.5, 'DisplayName', 'Predicted');
ylabel('Thrust (N)');
xlabel('Time (s)');
